...
def ListsToFillDataDict(uniqueListOfCodesInColumns):
    ...
    '''
    totalSize = 1       #length of unsparse version
    for List in uniqueListOfCodesInColumns:
        totalSize *= len(List)
    allList = []
    appendThisManyTimes = totalSize        #used to create unique combinations
    for List in uniqueListOfCodesInColumns:
        listToAppend = []
        appendThisManyTimes /= len(List)
        appendThisManyTimes = int(appendThisManyTimes)
        listDuplicatesTimes = int(totalSize / appendThisManyTimes / len(List))    #will duplicate the listToAppend
        for item in List:
